...
class BytesNmeaDownlinkConverter(NmeaConverter):
    def convert(self, config, data):
        try:
            ...
            if not isinstance(data, dict) or not data:
                log.error("Failed to convert TB data to NMEA payload: data is empty or not a dictionary")
                return
            ...
            if config.get("dataExpression", ""):
                ...
                log.error("Failed to convert TB data to NMEA payload: no `value` or `dataExpression` property")
                return
            ...
            log.error("Failed to convert TB data to NMEA payload: %s", str(e))
            return
