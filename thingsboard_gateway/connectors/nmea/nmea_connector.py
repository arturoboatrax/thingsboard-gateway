...
try:
    ...
    print("CAN library not found in NMEA Connector - installing...")
    TBUtility.install_package("python-can")
    from can import Notifier, BufferedReader, Message, CanError, ThreadSafeBus

from thingsboard_gateway.connectors.nmea.bytes_nmea_downlink_converter import BytesNmeaDownlinkConverter
from thingsboard_gateway.connectors.nmea.bytes_nmea_uplink_converter import BytesNmeaUplinkConverter
from thingsboard_gateway.connectors.connector import Connector, log


class NmeaConnector(Connector, Thread):
    ...
    def __init__(self, gateway, config, connector_type):
        self.statistics = {'MessagesReceived': 0,
                           'MessagesSent': 0}
        super().__init__()
        self.setName(config.get("name", 'Nmea Connector ' + ''.join(choice(ascii_lowercase) for _ in range(5))))
    ...
    def run(self):
        ...
        while need_run:
            ...
            try:
                ...
                process1 = subprocess.Popen(['candump', 'can0'],stdout=subprocess.PIPE)
                process2 = subprocess.Popen('candump2analyzer',stdin=process1.stdout, stdout=subprocess.PIPE)
                process3 = subprocess.Popen(['analyzer','-json'],stdin=process2.stdout, stdout=subprocess.PIPE)
                while True:
                    output = process3.stdout.readline()
                    if process3.poll() is not None and output == '':
                        break
                    if output:
                        #print (output.strip())
                        reader = output.strip()
                ...
                while not self.__stopped:
                    ...
                    if message is not None:
                        log.debug("[%s] New Nmea message received %s", self.get_name(), message)
                        log.error("[%s] New Nmea message received %s", self.get_name(), message.data)
                        self.__process_message(message)
                    self.__check_if_error_happened()
            except Exception as e:
                log.error("[%s] Error on Nmea bus: %s", self.get_name(), str(e))
            finally:
                try:
                    ...
                    if self.__bus is not None:
                        log.debug("[%s] Shutting down connection to Nmea bus (state=%s)",
                                  self.get_name(), self.__bus.state)
                        self.__bus.shutdown()
                except Exception as e:
                    log.error("[%s] Error on shutdown connection to Nmea bus: %s", self.get_name(), str(e))
    ...
    def send_data_to_bus(self, data, config, data_check=True, raise_exception=False):
        try:
            ...
            log.error("[%s] Wrong Nmea message data: %s", self.get_name(), str(e))
        except CanError as e:
            log.error("[%s] Failed to send Nmea message: %s", self.get_name(), str(e))
            if raise_exception:
                raise e
            else:
    ...
    def __on_bus_error(self, e):
        log.warning("[%s] Notified about Nmea bus error. Store it to later processing", self.get_name())
        self.__bus_error = e
    ...
    def __process_message(self, message):
        if message.arbitration_id not in self.__nodes:
            ...
            log.debug("[%s] Ignoring Nmea message. Unknown arbitration_id %d", self.get_name(), message.arbitration_id)
            return
        ...
        if cmd_id not in self.__nodes[message.arbitration_id]:
            log.debug("[%s] Ignoring Nmea message. Unknown cmd_id %d", self.get_name(), cmd_id)
            return

        log.debug("[%s] Processing Nmea message (id=%d,cmd_id=%s): %s",
        ...
        if data is None or not data.get("attributes", []) and not data.get("telemetry", []):
            log.warning("[%s] Failed to process Nmea message (id=%d,cmd_id=%s): data conversion failure",
                ...
                for msg_config in device_config[config_key]:
                    ...
                    if "polling" in msg_config:
                        try:
                            ...
                            # Create Nmea message object to validate its data
                            ...
                            log.warning("[%s] Ignore '%s' %s polling configuration, wrong Nmea data: %s",
...
class Poller(Thread):
    def __init__(self, connector: NmeaConnector):
        super().__init__()
    ...
    def __poll_and_schedule(self, data, config):
        ...
        log.debug("[%s] Sending periodic (%f sec) Nmea message (arbitration_id=%d, data=%s)",
